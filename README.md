...
从 `v1.2.0` 开始，基础信息收集阶段默认会尽可能详细地向用户提问，而不是优先依靠 Codex 自己推理。只有当用户明确要求快速生成、已经提供充分资料，或给出需要读取的文件路径时，Codex 才会做有限推断，并且必须把推断内容和未决问题标清楚。
...
生成前会先进行详细 intake，通常会询问课题名称、领域、目标、研究问题、研究对象、可用资源、方法路线、是否存在 baseline 或对照、研究方式、评价标准、目标产出、应用场景、约束条件、已有资料路径和输出路径。你可以回答“未定”“不适用”或“请给我几个选项”，skill 会把这些内容作为后续设计中的待决策点。
